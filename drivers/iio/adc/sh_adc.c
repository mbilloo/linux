...
static int sh_adc_probe(struct platform_device *pdev)
{
	...
	int numchan, mtuchan, i, nmap, nres, irq = 0;
	int ret = 0;
	...
	adc->reg = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(adc->reg))
		goto error_map;
	nmap++;
	/* MTU2 base address */
	res = platform_get_resource(pdev, IORESOURCE_MEM, nmap);
	adc->mtu2_share_reg = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(adc->mtu2_share_reg))
		goto error_map;
	nmap++;
	res = platform_get_resource(pdev, IORESOURCE_MEM, nmap);
	adc->mtu2_reg = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(adc->mtu2_reg))
		goto error_map;
	nmap++;
	...
	for (nres = 0; nres < pdev->num_resources; nres++) {
		...
		for (irq = res->start; irq <= res->end; irq++) {
			...
			//		  idev->name, idev);
			ret = request_irq(irq, sh_adc_irq, 0,
								  idev->name, idev);
			if (ret < 0) {
...
static const struct of_device_id of_sh_adc_match[] =
{
		{ .compatible = "renesas,sh-adc", },
		{ },
};

MODULE_DEVICE_TABLE( of, of_sh_adc_match);
...
static struct platform_driver sh_adc_driver = {
	...
	.driver = {
		.name = "sh_adc",
		.owner = THIS_MODULE,
		.of_match_table = of_sh_adc_match,
	},
};
