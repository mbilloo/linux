 ...
 *
 * This is a simple dma controller for transfering blocks from
 ...
 * 0x4 - status
 * 14 | 7 | 6 |    5 |  3   |  2  |      1     |     0       |
 *  ? | ? | ? | err? | done | int | get set when triggering  |
 *
...
#define STATUS_MAYBEERR		(1 << 4)
#define STATUS_DONE			(1 << 3)
#define STATUS_INT			(1 << 2)
...
static const struct regmap_config msc313_bdma_regmap_config = {
		.name = DRIVER_NAME,
		.reg_bits = 16,
		.val_bits = 16,
		.reg_stride = 4
...
static irqreturn_t msc313_bdma_irq(int irq, void *data)
{
	...
	regmap_write(chan->regmap, REG_STATUS, STATUS_MAYBEERR | STATUS_DONE | STATUS_INT);
	//printk("bdma int %04x\n", status);
...
static dma_cookie_t msc313_tx_submit(struct dma_async_tx_descriptor *tx){
	dma_cookie_t cookie;
	struct msc313_bdma_chan *chan = to_chan(tx->chan);
	struct msc313_bdma_desc *desc = to_desc(tx);
