...
#define REG_CURRENT_L          0x80
#define REG_CURRENT_H          0x84

static void msc313_cpuclk_setfreq(struct msc313e_cpuclk *cpuclk, u32 reg) {
	u16 l = reg & 0xffff, h = (reg >> 16) & 0xffff;
	iowrite16(l, cpuclk->base + REG_LPF_HIGH_BOTTOM);
	iowrite16(h, cpuclk->base + REG_LPF_HIGH_TOP);
	...
	iowrite16(l, cpuclk->base + REG_LPF_LOW_L);
	iowrite16(h, cpuclk->base + REG_LPF_LOW_H);
}
...
static unsigned long msc313e_cpuclk_frequencyforreg(u32 reg, unsigned long parent_rate){
	...
	unsigned long long scaled = DIV_ROUND_DOWN_ULL(prescaled, reg);
...
static u32 msc313e_cpuclk_regforfrequecy(unsigned long rate, unsigned long parent_rate){
	unsigned long long prescaled = ((unsigned long long)parent_rate) * (12 * 524288);
	unsigned long long scaler = DIV_ROUND_UP_ULL(prescaled, rate);
	u32 reg = scaler;
...
static unsigned long msc313e_cpuclk_recalc_rate(struct clk_hw *hw, unsigned long parent_rate){
	...
	u32 reg = ioread16(cpuclk->base + REG_LPF_LOW_H);
	reg = reg << 16;
	reg |= ioread16(cpuclk->base + REG_LPF_LOW_L);
	return msc313e_cpuclk_frequencyforreg(reg, parent_rate);
}
